package com.gjf.wherearethey_v2.databaseoperation.factory;

import com.gjf.wherearethey_v2.databaseoperation.dao.*;
...
public class DaoFactory {
    ...
    public static void initialize(Context context) {
        appContext = context;
        // 从共享偏好设置中读取配置的实现类型
        SharedUtil sharedUtil = SharedUtil.getInstance(context, "app_config");
    ...
    public static void setDefaultImplType(String implType) {
        ...
        // 保存到共享偏好设置
        if (appContext != null) {
            SharedUtil sharedUtil = SharedUtil.getInstance(appContext, "app_config");
    ...
    public static IUserDao getUserDao(String implType) {
        if (IMPL_TYPE_JDBC.equals(implType)) {
            return com.gjf.wherearethey_v2.databaseoperation.dao.UserDao.getInstance();
    ...
    public static INowLocationDao getNowLocationDao(String implType) {
        if (IMPL_TYPE_JDBC.equals(implType)) {
            return com.gjf.wherearethey_v2.databaseoperation.dao.NowLocationDao.getInstance();
        } else if (IMPL_TYPE_REST.equals(implType)) {
            // TODO: 实现REST API版本
            throw new UnsupportedOperationException("REST API implementation not yet available");
        } else {
    ...
    public static IMessageIODao getMessageIODao(String implType) {
        if (IMPL_TYPE_JDBC.equals(implType)) {
            return com.gjf.wherearethey_v2.databaseoperation.dao.MessageIODao.getInstance();
        } else if (IMPL_TYPE_REST.equals(implType)) {
            // TODO: 实现REST API版本
            throw new UnsupportedOperationException("REST API implementation not yet available");
        } else {
    ...
    public static IFriendsDao getFriendsDao(String implType) {
        if (IMPL_TYPE_JDBC.equals(implType)) {
            return com.gjf.wherearethey_v2.databaseoperation.dao.FriendsDao.getInstance();
        } else if (IMPL_TYPE_REST.equals(implType)) {
            // TODO: 实现REST API版本
            throw new UnsupportedOperationException("REST API implementation not yet available");
        } else {
